...
import {
  ...
  MessageSquare, Send, Trash2, User, Heart,
} from "lucide-react";
...
export default function ChapterReadPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setStory(s);
      setAllChapters(all ?? []);
      setChapterLikes((ch as any).likes_count ?? 0);
      setLoading(false);
    });

    // Restore chapter like from localStorage
    const liked: string[] = JSON.parse(localStorage.getItem("marginalia-chapter-liked") ?? "[]");
    setChapterLiked(liked.includes(chapterId));
  ...
  useEffect(() => {
    ...
    supabase.rpc("increment_chapter_views", { chapter_id: chapterId })
      .then(({ error }) => console.log("chapter views:", error ?? "OK"));
    supabase.rpc("increment_story_views", { sid: storyId })
      .then(({ error }) => console.log("story views:", error ?? "OK"));
  }, [chapterId, storyId]);
  ...
  async function toggleChapterLike() {
    ...
    const delta = chapterLiked ? -1 : 1;
    setChapterLiked(!chapterLiked);
    setChapterLikes((n) => n + delta);
    const liked: string[] = JSON.parse(localStorage.getItem("marginalia-chapter-liked") ?? "[]");
    const updated = chapterLiked ? liked.filter((id) => id !== chapterId) : [...liked, chapterId];
    localStorage.setItem("marginalia-chapter-liked", JSON.stringify(updated));
    await createClient().rpc("toggle_chapter_like", { cid: chapterId, delta });
  ...
  return (
    ...
    >
        ...
        <div className="mx-auto max-w-2xl px-5 pb-24 pt-6 sm:px-8">
          ...
          <div className="mb-10">
            ...
            <button
              ...
              className="mt-4 flex items-center gap-2 rounded-full border px-3 py-1.5 text-sm transition-all"
              style={{
                borderColor: chapterLiked ? "rgba(244,63,94,0.5)" : settings.textColor + "25",
                color: chapterLiked ? "#f43f5e" : settings.textColor,
                backgroundColor: chapterLiked ? "rgba(244,63,94,0.08)" : "transparent",
                opacity: chapterLikeBusy ? 0.6 : 1,
              }}
            >
              <Heart size={14} style={{ fill: chapterLiked ? "#f43f5e" : "none" }} />
              <span>{chapterLikes > 0 ? chapterLikes : ""} {chapterLiked ? "J'aime" : "J'aime"}</span>
            </button>
          </div>
            ...
            {settings.mode === "scroll" ? (
              isHtmlContent ? (
                <div
                  ...
                  className="rich-content [&_h2]:font-serif [&_h2]:font-bold [&_h2]:mt-6 [&_h2]:mb-3 [&_p]:mb-6 [&_strong]:font-bold [&_em]:italic [&_u]:underline"
                />
              ) : (
              ...
              <>
                {isHtmlContent ? (
                  <div
                    dangerouslySetInnerHTML={{ __html: pages[currentPage]?.join("") ?? "" }}
                    className="rich-content [&_h2]:font-serif [&_h2]:font-bold [&_h2]:mt-6 [&_h2]:mb-3 [&_p]:mb-6 [&_strong]:font-bold [&_em]:italic [&_u]:underline"
                  />
                ) : (
      ...
      {/* ── Floating play button (scroll mode only) ──────────────────────────── */}
      {settings.mode === "scroll" && (
        <div
          className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-2"
          style={{ pointerEvents: "none" }}
        >
          {/* Speed slider — only when playing */}
          {settings.autoScroll && (
            <div
              className="flex items-center gap-2 rounded-2xl border px-3 py-2 shadow-xl backdrop-blur-md"
              style={{
                backgroundColor: settings.bg + "ee",
                borderColor: settings.textColor + "20",
                pointerEvents: "auto",
              }}
            >
              <span className="text-[10px] opacity-40" style={{ color: settings.textColor }}>Lent</span>
              <input
                type="range" min={1} max={10} value={settings.scrollSpeed}
                onChange={(e) => setSettings((s) => ({ ...s, scrollSpeed: +e.target.value }))}
                className="w-24"
                style={{ accentColor: settings.textColor }}
              />
              <span className="text-[10px] opacity-40" style={{ color: settings.textColor }}>Rapide</span>
            </div>
          )}
          {/* Play / Pause pill */}
          <button
            onClick={() => setSettings((s) => ({ ...s, autoScroll: !s.autoScroll }))}
            className="flex items-center gap-2 rounded-full px-4 py-2.5 text-sm font-medium shadow-xl backdrop-blur-md transition-all hover:scale-105 active:scale-95"
            style={{
              backgroundColor: settings.autoScroll ? settings.textColor : settings.bg + "ee",
              color: settings.autoScroll ? settings.bg : settings.textColor,
              border: `1px solid ${settings.textColor}28`,
              pointerEvents: "auto",
            }}
            title={settings.autoScroll ? "Arrêter le défilement" : "Défilement automatique"}
          >
            {settings.autoScroll ? <Pause size={15} /> : <Play size={15} />}
            <span className="text-xs">{settings.autoScroll ? "Pause" : "Lire"}</span>
          </button>
        </div>
      )}
    </div>
                  ...
                  <div className="flex items-center justify-between gap-2 mb-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      {authUser && authUser.id !== comment.user_id && (
                        <ReportButton contentType="comment" contentId={comment.id} compact />
                      )}
