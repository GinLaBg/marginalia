...
import { ArrowLeft, BookOpen, Clock, Eye, Heart, Star } from "lucide-react";
...
type Chapter = { id: string; title: string; order: number; published_at: string };
...
export default function StoryPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from("chapters").select("id, title, order, published_at").eq("story_id", storyId).eq("is_published", true).order("order", { ascending: true }),
    ]).then(([{ data: s }, { data: c }]) => {
      setStory(s);
      setChapters(c ?? []);
      setLoading(false);
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: "easeOut" }}>
        ...
        <div className="flex flex-col gap-8 sm:flex-row sm:items-start">
          ...
          <div className="flex-1 min-w-0">
            ...
            {story.author_name && <p className="mt-2 text-sm text-muted-foreground">par {story.author_name}</p>}
            ...
            {chapters.length > 0 && (
              <Link
                ...
                className={cn(buttonVariants({ size: "lg" }), "mt-6 bg-[var(--accent)] text-white hover:bg-[var(--accent)]/90")}
              >
                <BookOpen size={16} />
        ...
        {chapters.length > 1 && (
          <div className="mt-12">
            <h2 className="mb-4 font-serif text-2xl">Chapitres</h2>
            <div className="space-y-2">
              {chapters.map((chapter, index) => (
                <Link
                  key={chapter.id}
                  href={`/lire/${story.id}/${chapter.id}`}
                  className="flex items-center justify-between rounded-2xl border border-border bg-card px-5 py-4 transition-all hover:border-[var(--accent)]/40 hover:-translate-y-0.5"
                >
                  <div>
